...
import { stripMcCodes } from "../utils/Minecraft";
import { useGameData } from '../hooks/useGameData';
import { useDebounce } from '../hooks/useDebounce';
import { getTextureWithFallback } from "../utils/Texture-Finder";
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p
                className="text-sm font-medium truncate group-hover:text-[#FFAA00] transition-colors"
                style={{ color: r.hex }}
            >
              {item.displayName}
            </p>
...
function getRecipeGrid(matrix) {
  ...
  for (let i = 0; i < 3; i++) {
    grid.push(matrix.slice(i * 3, i * 3 + 3));
  }
      ...
      >
        ...
        >
          ...
          {tooltip && createPortal(
              <div
                  ...
                  style={{
                    ...
                    zIndex: 9999,
                    color: RARITY_COLORS[tooltip.rarity]?.hex || '#AAAAAA',
                    background: '#111',
                    borderColor: RARITY_COLORS[tooltip.rarity]?.hex + '66' || '#333',
                  }}
              >
                {tooltip.text}
              </div>,
          ...
          <div className="p-4 space-y-4">
            ...
            <div
              style={{
              maxHeight: showRecipe ? '500px' : '0px',
              overflow: 'hidden',
...
export default function ItemsPage() {
  ...
  const categories = useMemo(() =>
          flatItems
              ? [...new Set(Object.values(flatItems).map(i => i.category))].sort()
              : [],
      [flatItems]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return Object.entries(flatItems)
        .filter(([, item]) => {
          const matchSearch = !debouncedSearch || item.displayName?.toLowerCase().includes(debouncedSearch.toLowerCase());
        ...
        .sort((a, b) => {
          ...
          switch (sortBy) {
            ...
            case 'rarity':
            default: return rarityB - rarityA;
          }
        });
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 py-8">
          ...
          <div className="flex flex-wrap gap-3 mb-6" data-testid="items-filters">
            ...
            >
              ...
              {rarities.map(r => (
                  <option key={r} value={r}>{RARITY_COLORS[r]?.label || r}</option>
              ))}
            </select>
